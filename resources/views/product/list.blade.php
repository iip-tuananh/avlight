                            ...
                            <form action="{{ route('add.to.cart') }}" method="post" class="variants product-action"
                                ...
                                <div class="blog-item wow fadeInUp tuanproduct" data-wow-delay=".25s">
                                    <div class="blog-item-img">
                                        <a
                                            href="{{ route('detailProduct', ['cate' => $item->cate_slug ?? $item->cate->slug, 'type' => $item->type_slug ?? $item->typecate->slug, 'id' => $item->id]) }}">
                                            <img class="lazy"
                                    ...
                                    <div class="blog-item-info" style="text-align: center">
                                        <h4 class="blog-title">
                                            <a class="line-camp-2 text-t"
                                                href="{{ route('detailProduct', ['cate' => $item->cate_slug ?? $item->cate->slug, 'type' => $item->type_slug ?? $item->typecate->slug, 'id' => $item->id]) }}">{!! languageName($item->name) !!}</a>
                                        </h4>
                                        @if ($item->price > 0)
                                            @if ($item->discount > 0)
                                        ...
                                        @else
                                            ...
                                            <div class="theme-btn " style="background-color:#025aa1">
                                                <a href="{{ route('detailProduct', ['cate' => $item->cate_slug ?? $item->cate->slug, 'type' => $item->type_slug ?? $item->typecate->slug, 'id' => $item->id]) }}"
