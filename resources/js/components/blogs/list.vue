							...
							<template slot-scope="{ data }">
								<vs-tr :key="indextr" v-for="(tr, indextr) in data">
									...
									<vs-td v-if="tr.cate != null">
										{{ JSON.parse(tr.cate.name)[0].content }}
									</vs-td>
									<vs-td v-if="tr.cate == null">--Trống--</vs-td>
									<vs-td v-if="tr.type_cate != null">
										{{ JSON.parse(tr.type_cate.name)[0].content }}
									</vs-td>
									<vs-td v-if="tr.type_cate == null">-----</vs-td>
									<vs-td v-if="tr.type_news == 'tin-hot'">Tin Hot</vs-td>
									<vs-td v-if="tr.type_news == 'tin-khuyen-mai'">
										Tin Khuyến Mãi
									</vs-td>
									<vs-td v-if="tr.type_news == null">-----</vs-td>
									<vs-td>
										<router-link
											:to="{ name: 'editBlog', params: { id: tr.id } }"
										>
											<vs-button
												v-if="tr"
												vs-type="gradient"
