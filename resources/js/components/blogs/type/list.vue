			...
			<div class="col-md-12 grid-margin stretch-card">
				<div class="card">
					<div class="card-body">
						...
						<vs-button type="gradient" style="float: right" @click="popupActivo = true">
							Thêm mới
						</vs-button>
						<vs-input
							icon="search"
						...
						<vs-table max-items="5" pagination :data="list">
							...
							<template slot-scope="{ data }">
								<vs-tr :key="indextr" v-for="(tr, indextr) in data">
									...
									<vs-td :data="tr.id">
										{{ JSON.parse(tr.cate_blog.name)[0].content }}
									</vs-td>
									<vs-td :data="tr.id">
										<router-link
											:to="{ name: 'editTypeBlog', params: { id: tr.id } }"
										>
											<vs-button
												vs-type="gradient"
												size="lagre"
...
export default {
	...
	methods: {
		...
		listTypelogs() {
			this.loadings(true);
			this.listTypelog({ keyword: this.keyword }).then(response => {
		...
		destroy() {
			this.loadings(true);
			this.destroyTypeBLog({ id: this.id_item }).then(response => {
				this.listTypelogs();
				this.loadings(false);
