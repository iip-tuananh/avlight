						...
						<vs-table max-items="5" pagination :data="list">
							...
							<template slot-scope="{ data }">
								<vs-tr :key="indextr" v-for="(tr, indextr) in data">
									...
									<vs-td :data="tr.type" v-if="tr.type == 'ho-tro-khanh-hang'">
										Hỗ trợ chúng tôi
									</vs-td>
									<vs-td :data="tr.type" v-if="tr.type == 've-chung-toi'">
										Về chúng tôi
									</vs-td>
									<vs-td :data="tr.id">
										<router-link
										...
										<vs-button
											...
											color="red"
											icon="delete_forever"
											@click="confirmDestroy(tr.quiz_id)"
										></vs-button>
									</vs-td>
								</vs-tr>
							</template>
...
export default {
	...
	methods: {
		...
		listPageContents() {
			this.loadings(true);
			this.listPageContent({ keyword: this.keyword }).then(response => {
				this.loadings(false);
		...
		destroy() {
			this.loadings(true);
			this.deletePageContent(this.id_item).then(response => {
