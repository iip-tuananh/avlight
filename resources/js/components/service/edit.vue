				...
				<div class="card">
					<div class="card-body">
                        ...
                        <div class="form-group">
							...
							<vs-select v-model="objData.cate_id">
								<vs-select-item v-for="item in cate" :key="item.id" :value="item.id" :text="item.name" />
							</vs-select>
						</div>
					</div>
				</div>
...
export default {
	...
	methods: {
			...
			if (this.errors.length > 0) {
				...
				this.addService(this.objData)
					.then(response => {
						...
						this.$notify.success('Cập nhật dịch vụ thành công');
					})
					.catch(error => {
						this.loadings(false);
						this.$notify.error('Cập nhật dịch vụ thất bại');
					});
			}
		},
		...
		editById() {
			this.loadings(true);
			this.detailService(this.objData)
				.then(response => {
					this.loadings(false);
